...
export const fetchOurVision = async (token) => {
  try {
    const response = await axios.get(`${baseUrl}/api/vision-list`, {
      headers: { Authorization: `Bearer ${token}` },
    });

    if (response.data?.vision_list) {
      return response.data.vision_list;
    } else {
      throw new Error("Invalid Vision data");
    }
  } catch (error) {
